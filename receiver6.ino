...
void loop() {                                  //---------- A D A P T I V E - S Y N C ----------
    for(int i=0; i<3; i++){                    //for 3 times
        if (f2 == 0){                          //if transmitter and receiver are in sync(state:0)
        ...
        }else if(f2 == 1){                     //if transmitter is ahead of time(state:1)
            if (i != 2){                       //for the first 2 times
                reading = analogRead(lightPin);//read input 
                if (reading > (ambient + 100)){//if input value is greater than ambient by a 100
                    n1 += 1;                   //a "1" was found so increase n1 by 1
                    if(i==0){                  //if the first reading
            ...
            if (i == 0){                       //if it is the first reading
                delay(dl);                     //wait dl msec
    ...
    if (n1>=n0){         //if more "1"s were found
        ...
        s_final = "1";  //
        if (n0 == 1){   //if a 0 was found once
            if(f1==1){  //if 1 was found at the first reading(0 was found at the last reading)
                f2 = 1; //transmitter is ahead of time
            }else{      //if 1 was not found at the first reading(0 was found at the first reading)
        ...
        s_final = "0";  //
        if (n1 == 1){   //if a 1 was found once
            if(f0==1){  //if 0 was found at the first reading(1 was found at the last reading)
                f2 = 1; //transmitter is ahead of time
            }else{      //if 0 was not found at the first reading(1 was found at the first reading)
                f2 = 2; //receiver is ahead of time
            }           //
            f0 = 0;     //reset f0
        }               //
    ...
    if (start_flag == 0){                       //if message has not been detected (state: 0)
        ...
        if (j>=0){                    //for every bit of the 6 starting from the left-most(MSB: most significant bit)
            if(n_final == 1){         //if decoded bit has a value of "1" 
                bitSet(msg_length, j);//set the "j"bit (of msg_length
            }                         // variable) value as "1"
            if (j>0){                 //ex: j=4 (msg_length is initialized as 0 ) 
                j--;                  //000000 --> 001000 binary or 16 decimal(msg_length is integer so it is auto-converted)
            }else{                    //when the 6th bit is decoded before moving on with the next bit 
        ...
        if(msg_length>0){                                     //for every character
            if (k>=0){                                        //for every bit of the current character starting from the MSB
                if(n_final == 1){                             //if decoded bit has a value of "1" 
                    bitSet(x, k);                             //set the "j"bit (of x variable)value as "1"
                }                                             //
                if (k>0){                                     //
                    k--;                                      //
                }else{                                        //when the 8th bit is decoded before moving on with the next bit 
