...
void decodeBit() {
  ...
  if (countOne >= countZero) {  // if more "1"s were found
    decodedBit = "1";           //
    if (countZero == 1) {       // if a 0 was found once
      currentSyncState = firstReadingOne ? syncState::TransmitterFirst : syncState::ResceiverFirst;
    }
  } else {  // else(more "0"s were found)
    decodedBit = "0";

    if (countOne == 1) {  // if a 1 was found once
      currentSyncState = !firstReadingOne ? syncState::TransmitterFirst : syncState::ResceiverFirst;
    }
  }
...
void detectMsgStart() {                                                                                                                                                                     // if message has not been detected (state: 0)
...
void decodeMsgLength() {
  ...
  if (decodedBit.compareTo("1") == 0) {    // if decoded bit has a value of "1"
    bitSet(msgLength, lengthBitPosition);  // set the "j"bit (of msg_length
  }                                        // variable) value as "1"
  if (lengthBitPosition > 0) {             // ex: j=4 (msg_length is initialized as 0 )
