...
void loop() {                           //---------- D E C O D E - B I T ----------
    ...
    if(start_flag == 0){                        //---------- S Y N C H R O N I Z A T I O N ----------
        dequeue();                              //if message has not been detected (state: 0)
        ...
        if (j>=0){                    //for every bit of the 8 starting from the left-most (MSB: most significant bit)
            if(n_final == 1){         //if decoded bit has a value of "1"
                bitSet(msg_length, j);//calculate 2^j and add the result 
            }                         //to msg_length variable
            j--;                      //
        ...
        if(msg_length>0){                                 //for every character
            if (j>=0){                                    //for every bit of the current character starting from the MSB
                if(n_final == 1){                         //if decoded bit has a value of "1"
                    bitSet(x, j);                         //calculate 2^j and add the 
